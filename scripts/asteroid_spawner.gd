...
@onready var game_manager: Node = %"game manager" # Adjust the path if needed
...
func spawn_asteroid(position: Vector2):
	...
	# Assign the game manager to the asteroid
	if asteroid.has_method("set"):  # safety check, but optional
